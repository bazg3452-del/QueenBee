...
攻防能力不来自从零开发的算法，而来自对成熟组件的编排。成熟的 agent 工具与成熟的渗透技能库已经具备单题解题能力；真正没有现成答案的是组织问题——调度、计时、交接与回收。

QueenBee 只解决组织问题：引擎不调用模型，用确定性代码完成容器管理、进程派发与超时控制；解题完全交由成熟 agent 执行。

技能与方法论是约束：约束带来稳定，也截断创造力。因此每道题并行两名 agent——一名按技能库与方法论推进，一名不施加任何约束、自由发挥；两者产出经黑板互相可见，任一 agent 被替换时不损失既有情报。
